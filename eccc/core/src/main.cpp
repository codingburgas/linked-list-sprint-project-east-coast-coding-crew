...
void displayCommandHelp() {
	...
	std::cout << GREEN << "> event category <name>" << RESET << " Find events by category\n";
	std::cout << GREEN << "> event location <name>" << RESET << " Find events by location\n";
...
void addSampleData(HistoricalDbManager& dbManager) {
...
HistoricalEvent inputEventDetails() {
...
int main() {
	...
	if (!dbHandle) {
		std::cerr << RED << "✗ Failed to get database instance: " << dbHandle.error() << RESET << "\n";
		return 1;
	}
	...
	if (!connectionResult) {
		std::cerr << RED << "✗ Failed to connect to database: " << connectionResult.error() << RESET << "\n";
		return 1;
	}

	std::cout << GREEN << "✓ " << RESET << "Connected to the database\n";

	HistoricalDbManager dbManager(db);
	...
	while (running) {
		...
		if (command == "exit" || command == "quit") {
			...
			if (subCommand == "add") {
				...
				if (args.size() < 2) {
					std::cout << RED << "✗ " << RESET << "Missing category name. Usage: event category <name>\n";
					continue;
				}
				...
				if (args.size() < 2) {
					std::cout << RED << "✗ " << RESET << "Missing location name. Usage: event location <name>\n";
					continue;
				}
